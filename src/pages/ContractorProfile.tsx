...
const CONTRACTOR_QUERY_KEY = "contractor";

const ContractorProfile = () => {
  const navigate = useNavigate();
  const { projectId, contractorId } = useParams();
  const { data: contractor, isFetching } = useQuery({
    queryKey: [CONTRACTOR_QUERY_KEY, projectId, contractorId],
    enabled: !!projectId && !!contractorId,
    queryFn: () => getMemberByIdApi(projectId!, contractorId!),
    ...
    <main className="space-y-6">
          ...
          <div className="flex h-auto w-full items-center justify-start space-x-3 p-2">
            ...
            <div>
              ...
              <p className="text-lg text-gray-500 capitalize">
                {formatCamelCaseToText(contractor?.role)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 text-sm lg:grid-cols-1">
            <div className="space-y-2">
              {contractor.responsibilities.map((responsibility) => (
                <p key={responsibility} className="font-semibold text-gray-900">
                  {responsibility}
                </p>
              ))}
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 text-sm lg:grid-cols-1">
            <div className="space-y-2">
              {contractor.permissions.map((permissions) => (
